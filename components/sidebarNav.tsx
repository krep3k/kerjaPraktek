    ...
    return (
        <nav className="px-3 space-y-1">
            {filteredNavs.map((nav) => {
                ...
                return (
                    <Link key={nav.name} href={nav.href} title={isCollapsed ? nav.name : ""} className={`flex items-center rounded-2xl transition-all duration-200 group ${isCollapsed ? "justify-center p-3.5" : "gap-4 px-5 py-3.5"} ${isActive ? "bg-[#4455ed] text-[#e7e7e7] shadow-lg" : "text-[#6E6B7F] hover:bg-[#81CAD6]/10 hover:text-[#2D2735]"}`}>
